.class public Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Ljava/io/DataInput;


# instance fields
.field private back:B

.field private final byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

.field private byteSourcePosition:J

.field private isBack:Z


# direct methods
.method public constructor <init>(Lcom/itextpdf/text/io/RandomAccessSource;)V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    iput-object p1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    return-void
.end method

.method public constructor <init>(Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;)V
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/IndependentRandomAccessSource;

    iget-object p1, p1, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    invoke-direct {v0, p1}, Lcom/itextpdf/text/io/IndependentRandomAccessSource;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    invoke-direct {p0, v0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method

.method public constructor <init>(Ljava/io/InputStream;)V
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    invoke-direct {v0}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;-><init>()V

    invoke-virtual {v0, p1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->createSource(Ljava/io/InputStream;)Lcom/itextpdf/text/io/RandomAccessSource;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method

.method public constructor <init>(Ljava/lang/String;)V
    .locals 2
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    invoke-direct {v0}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;-><init>()V

    const/4 v1, 0x0

    invoke-virtual {v0, v1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->setForceRead(Z)Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    move-result-object v0

    sget-boolean v1, Lcom/itextpdf/text/Document;->plainRandomAccess:Z

    invoke-virtual {v0, v1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->setUsePlainRandomAccess(Z)Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    move-result-object v0

    invoke-virtual {v0, p1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->createBestSource(Ljava/lang/String;)Lcom/itextpdf/text/io/RandomAccessSource;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method

.method public constructor <init>(Ljava/lang/String;ZZ)V
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    invoke-direct {v0}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;-><init>()V

    invoke-virtual {v0, p2}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->setForceRead(Z)Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    move-result-object p2

    invoke-virtual {p2, p3}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->setUsePlainRandomAccess(Z)Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    move-result-object p2

    invoke-virtual {p2, p1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->createBestSource(Ljava/lang/String;)Lcom/itextpdf/text/io/RandomAccessSource;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method

.method public constructor <init>(Ljava/net/URL;)V
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    invoke-direct {v0}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;-><init>()V

    invoke-virtual {v0, p1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->createSource(Ljava/net/URL;)Lcom/itextpdf/text/io/RandomAccessSource;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method

.method public constructor <init>([B)V
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    new-instance v0, Lcom/itextpdf/text/io/RandomAccessSourceFactory;

    invoke-direct {v0}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;-><init>()V

    invoke-virtual {v0, p1}, Lcom/itextpdf/text/io/RandomAccessSourceFactory;->createSource([B)Lcom/itextpdf/text/io/RandomAccessSource;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-void
.end method


# virtual methods
.method public close()V
    .locals 1

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    iget-object v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    invoke-interface {v0}, Lcom/itextpdf/text/io/RandomAccessSource;->close()V

    return-void
.end method

.method public createSourceView()Lcom/itextpdf/text/io/RandomAccessSource;
    .locals 2

    new-instance v0, Lcom/itextpdf/text/io/IndependentRandomAccessSource;

    iget-object v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    invoke-direct {v0, v1}, Lcom/itextpdf/text/io/IndependentRandomAccessSource;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-object v0
.end method

.method public createView()Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;
    .locals 3

    new-instance v0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;

    new-instance v1, Lcom/itextpdf/text/io/IndependentRandomAccessSource;

    iget-object v2, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    invoke-direct {v1, v2}, Lcom/itextpdf/text/io/IndependentRandomAccessSource;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    invoke-direct {v0, v1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;-><init>(Lcom/itextpdf/text/io/RandomAccessSource;)V

    return-object v0
.end method

.method public getByteSource()Lcom/itextpdf/text/io/RandomAccessSource;
    .locals 1
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    iget-object v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    return-object v0
.end method

.method public getFilePointer()J
    .locals 4

    iget-wide v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    iget-boolean v2, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    int-to-long v2, v2

    sub-long/2addr v0, v2

    return-wide v0
.end method

.method public length()J
    .locals 2

    iget-object v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    invoke-interface {v0}, Lcom/itextpdf/text/io/RandomAccessSource;->length()J

    move-result-wide v0

    return-wide v0
.end method

.method public pushBack(B)V
    .locals 0

    iput-byte p1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->back:B

    const/4 p1, 0x1

    iput-boolean p1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    return-void
.end method

.method public reOpen()V
    .locals 2
    .annotation runtime Ljava/lang/Deprecated;
    .end annotation

    const-wide/16 v0, 0x0

    invoke-virtual {p0, v0, v1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->seek(J)V

    return-void
.end method

.method public read()I
    .locals 5

    iget-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    if-eqz v0, :cond_0

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    iget-byte v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->back:B

    and-int/lit16 v0, v0, 0xff

    return v0

    :cond_0
    iget-object v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    iget-wide v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    const-wide/16 v3, 0x1

    add-long/2addr v3, v1

    iput-wide v3, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    invoke-interface {v0, v1, v2}, Lcom/itextpdf/text/io/RandomAccessSource;->get(J)I

    move-result v0

    return v0
.end method

.method public read([B)I
    .locals 2

    array-length v0, p1

    const/4 v1, 0x0

    invoke-virtual {p0, p1, v1, v0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read([BII)I

    move-result p1

    return p1
.end method

.method public read([BII)I
    .locals 7

    const/4 v0, 0x0

    if-nez p3, :cond_0

    return v0

    :cond_0
    iget-boolean v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    if-eqz v1, :cond_1

    if-lez p3, :cond_1

    iput-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    add-int/lit8 v0, p2, 0x1

    iget-byte v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->back:B

    aput-byte v1, p1, p2

    add-int/lit8 p3, p3, -0x1

    const/4 p2, 0x1

    move v6, p3

    move v5, v0

    const/4 v0, 0x1

    goto :goto_0

    :cond_1
    move v5, p2

    move v6, p3

    :goto_0
    if-lez v6, :cond_2

    iget-object v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSource:Lcom/itextpdf/text/io/RandomAccessSource;

    iget-wide v2, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    move-object v4, p1

    invoke-interface/range {v1 .. v6}, Lcom/itextpdf/text/io/RandomAccessSource;->get(J[BII)I

    move-result p1

    if-lez p1, :cond_2

    add-int/2addr v0, p1

    iget-wide p2, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    int-to-long v1, p1

    add-long/2addr p2, v1

    iput-wide p2, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    :cond_2
    if-nez v0, :cond_3

    const/4 p1, -0x1

    return p1

    :cond_3
    return v0
.end method

.method public readBoolean()Z
    .locals 1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    if-ltz v0, :cond_1

    if-eqz v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0

    :cond_1
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readByte()B
    .locals 1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    if-ltz v0, :cond_0

    int-to-byte v0, v0

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readChar()C
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v0, v0, 0x8

    add-int/2addr v0, v1

    int-to-char v0, v0

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readCharLE()C
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v1, v1, 0x8

    add-int/2addr v1, v0

    int-to-char v0, v1

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readDouble()D
    .locals 2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readLong()J

    move-result-wide v0

    invoke-static {v0, v1}, Ljava/lang/Double;->longBitsToDouble(J)D

    move-result-wide v0

    return-wide v0
.end method

.method public final readDoubleLE()D
    .locals 2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readLongLE()J

    move-result-wide v0

    invoke-static {v0, v1}, Ljava/lang/Double;->longBitsToDouble(J)D

    move-result-wide v0

    return-wide v0
.end method

.method public readFloat()F
    .locals 1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readInt()I

    move-result v0

    invoke-static {v0}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v0

    return v0
.end method

.method public final readFloatLE()F
    .locals 1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readIntLE()I

    move-result v0

    invoke-static {v0}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v0

    return v0
.end method

.method public readFully([B)V
    .locals 2

    array-length v0, p1

    const/4 v1, 0x0

    invoke-virtual {p0, p1, v1, v0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readFully([BII)V

    return-void
.end method

.method public readFully([BII)V
    .locals 3

    const/4 v0, 0x0

    :cond_0
    add-int v1, p2, v0

    sub-int v2, p3, v0

    invoke-virtual {p0, p1, v1, v2}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read([BII)I

    move-result v1

    if-ltz v1, :cond_1

    add-int/2addr v0, v1

    if-lt v0, p3, :cond_0

    return-void

    :cond_1
    new-instance p1, Ljava/io/EOFException;

    invoke-direct {p1}, Ljava/io/EOFException;-><init>()V

    throw p1
.end method

.method public readInt()I
    .locals 5

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v3

    or-int v4, v0, v1

    or-int/2addr v4, v2

    or-int/2addr v4, v3

    if-ltz v4, :cond_0

    shl-int/lit8 v0, v0, 0x18

    shl-int/lit8 v1, v1, 0x10

    add-int/2addr v0, v1

    shl-int/lit8 v1, v2, 0x8

    add-int/2addr v0, v1

    add-int/2addr v0, v3

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readIntLE()I
    .locals 5

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v3

    or-int v4, v0, v1

    or-int/2addr v4, v2

    or-int/2addr v4, v3

    if-ltz v4, :cond_0

    shl-int/lit8 v3, v3, 0x18

    shl-int/lit8 v2, v2, 0x10

    add-int/2addr v3, v2

    shl-int/lit8 v1, v1, 0x8

    add-int/2addr v3, v1

    add-int/2addr v3, v0

    return v3

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readLine()Ljava/lang/String;
    .locals 8

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v1, -0x1

    const/4 v2, 0x0

    const/4 v3, -0x1

    :goto_0
    if-nez v2, :cond_2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v3

    const/4 v4, 0x1

    if-eq v3, v1, :cond_1

    const/16 v5, 0xa

    if-eq v3, v5, :cond_1

    const/16 v6, 0xd

    if-eq v3, v6, :cond_0

    int-to-char v4, v3

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    goto :goto_0

    :cond_0
    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->getFilePointer()J

    move-result-wide v6

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v2

    if-eq v2, v5, :cond_1

    invoke-virtual {p0, v6, v7}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->seek(J)V

    :cond_1
    const/4 v2, 0x1

    goto :goto_0

    :cond_2
    if-ne v3, v1, :cond_3

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->length()I

    move-result v1

    if-nez v1, :cond_3

    const/4 v0, 0x0

    return-object v0

    :cond_3
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public readLong()J
    .locals 6

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readInt()I

    move-result v0

    int-to-long v0, v0

    const/16 v2, 0x20

    shl-long/2addr v0, v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readInt()I

    move-result v2

    int-to-long v2, v2

    const-wide v4, 0xffffffffL

    and-long/2addr v2, v4

    add-long/2addr v0, v2

    return-wide v0
.end method

.method public final readLongLE()J
    .locals 7

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readIntLE()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readIntLE()I

    move-result v1

    int-to-long v1, v1

    const/16 v3, 0x20

    shl-long/2addr v1, v3

    int-to-long v3, v0

    const-wide v5, 0xffffffffL

    and-long/2addr v3, v5

    add-long/2addr v1, v3

    return-wide v1
.end method

.method public readShort()S
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v0, v0, 0x8

    add-int/2addr v0, v1

    int-to-short v0, v0

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readShortLE()S
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v1, v1, 0x8

    add-int/2addr v1, v0

    int-to-short v0, v1

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readString(ILjava/lang/String;)Ljava/lang/String;
    .locals 1

    new-array p1, p1, [B

    invoke-virtual {p0, p1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->readFully([B)V

    :try_start_0
    new-instance v0, Ljava/lang/String;

    invoke-direct {v0, p1, p2}, Ljava/lang/String;-><init>([BLjava/lang/String;)V
    :try_end_0
    .catch Ljava/lang/Exception; {:try_start_0 .. :try_end_0} :catch_0

    return-object v0

    :catch_0
    move-exception p1

    new-instance p2, Lcom/itextpdf/text/ExceptionConverter;

    invoke-direct {p2, p1}, Lcom/itextpdf/text/ExceptionConverter;-><init>(Ljava/lang/Exception;)V

    throw p2
.end method

.method public readUTF()Ljava/lang/String;
    .locals 1

    invoke-static {p0}, Ljava/io/DataInputStream;->readUTF(Ljava/io/DataInput;)Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public readUnsignedByte()I
    .locals 1

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    if-ltz v0, :cond_0

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readUnsignedInt()J
    .locals 13

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    int-to-long v0, v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v2

    int-to-long v2, v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v4

    int-to-long v4, v4

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v6

    int-to-long v6, v6

    or-long v8, v0, v2

    or-long/2addr v8, v4

    or-long/2addr v8, v6

    const-wide/16 v10, 0x0

    cmp-long v12, v8, v10

    if-ltz v12, :cond_0

    const/16 v8, 0x18

    shl-long/2addr v0, v8

    const/16 v8, 0x10

    shl-long/2addr v2, v8

    add-long/2addr v0, v2

    const/16 v2, 0x8

    shl-long v2, v4, v2

    add-long/2addr v0, v2

    add-long/2addr v0, v6

    return-wide v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readUnsignedIntLE()J
    .locals 13

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    int-to-long v0, v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v2

    int-to-long v2, v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v4

    int-to-long v4, v4

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v6

    int-to-long v6, v6

    or-long v8, v0, v2

    or-long/2addr v8, v4

    or-long/2addr v8, v6

    const-wide/16 v10, 0x0

    cmp-long v12, v8, v10

    if-ltz v12, :cond_0

    const/16 v8, 0x18

    shl-long/2addr v6, v8

    const/16 v8, 0x10

    shl-long/2addr v4, v8

    add-long/2addr v6, v4

    const/16 v4, 0x8

    shl-long/2addr v2, v4

    add-long/2addr v6, v2

    add-long/2addr v6, v0

    return-wide v6

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public readUnsignedShort()I
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v0, v0, 0x8

    add-int/2addr v0, v1

    return v0

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public final readUnsignedShortLE()I
    .locals 3

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v0

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->read()I

    move-result v1

    or-int v2, v0, v1

    if-ltz v2, :cond_0

    shl-int/lit8 v1, v1, 0x8

    add-int/2addr v1, v0

    return v1

    :cond_0
    new-instance v0, Ljava/io/EOFException;

    invoke-direct {v0}, Ljava/io/EOFException;-><init>()V

    throw v0
.end method

.method public seek(J)V
    .locals 0

    iput-wide p1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->byteSourcePosition:J

    const/4 p1, 0x0

    iput-boolean p1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    return-void
.end method

.method public skip(J)J
    .locals 6

    const-wide/16 v0, 0x0

    cmp-long v2, p1, v0

    if-gtz v2, :cond_0

    return-wide v0

    :cond_0
    iget-boolean v0, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    const/4 v1, 0x0

    if-eqz v0, :cond_2

    iput-boolean v1, p0, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->isBack:Z

    const-wide/16 v0, 0x1

    cmp-long v2, p1, v0

    if-nez v2, :cond_1

    return-wide v0

    :cond_1
    sub-long/2addr p1, v0

    const/4 v1, 0x1

    :cond_2
    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->getFilePointer()J

    move-result-wide v2

    invoke-virtual {p0}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->length()J

    move-result-wide v4

    add-long/2addr p1, v2

    cmp-long v0, p1, v4

    if-lez v0, :cond_3

    goto :goto_0

    :cond_3
    move-wide v4, p1

    :goto_0
    invoke-virtual {p0, v4, v5}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->seek(J)V

    sub-long/2addr v4, v2

    int-to-long p1, v1

    add-long/2addr v4, p1

    return-wide v4
.end method

.method public skipBytes(I)I
    .locals 2

    int-to-long v0, p1

    invoke-virtual {p0, v0, v1}, Lcom/itextpdf/text/pdf/RandomAccessFileOrArray;->skip(J)J

    move-result-wide v0

    long-to-int p1, v0

    return p1
.end method
